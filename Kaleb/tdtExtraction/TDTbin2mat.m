...
for i = 1:numel(headerStruct.uniqueCodes)
    ...
    if isequal(currentType, 'epocs')
        if strcmp(currentName(end), '\')
            ...
            data.epocs.(currentName).data = headerStruct.values(ind)';
            data.epocs.(currentName).onset = headerStruct.timestamps(ind)';
    ...
    elseif isequal(currentType, 'scalars')
        ...
        for xx = 1:nchan
            data.scalars.(currentName).data(xx,:) = headerStruct.values(ind(headerStruct.channels(ind) == xx));
        end
        
        % only use channel 1 timestamps
        data.scalars.(currentName).ts = headerStruct.timestamps(ind(headerStruct.channels(ind) == 1));
    ...
    elseif isequal(currentType, 'snips')
        ...
        all_sizes = headerStruct.sizes(ind);        
        ...
        data.snips.(currentName).chan = headerStruct.channels(ind)';
        data.snips.(currentName).sortcode = headerStruct.sortcodes(ind)';
        data.snips.(currentName).ts = headerStruct.timestamps(ind)';
    ...
    elseif isequal(currentType, 'streams')
        ...
        if useSEVs
            ...
            data.streams.(currentName).fs = headerStruct.freqs(ind(1));
